...
av        = 0.8;          %niveau insnoering in verticale richting NB (0.001<= av <=1): av=0 geeft rare antwoorden (av = 0.001 niet)
...
% topduur_inv = ...       %Keuze voor PR1371.30 (B=60 dagen)
...
%    1.55, 1.01e-8/Ntrapezia]   %ingeval ovfreq in plaatjes verder door moet lopen

%Tbv verkrijgen dagenlijn Hydra-M (N.B.: uitgangspunt is dat P(M>m) in
%Hydra-M wordt verkregen door delen door OD_HM(1), en niet door delen door 182!!!
%B=60 dagen, av=0.8, ah=0.025
topduur_inv = ...       
    [-0.40, B*24;        %eerste kolom moet toenemend zijn, voor tweede geldt dat niet
    -0.22, 192;
     0.2,  72;
...
v = standaardvorm.v;
bpiek_fig   = topduur_inv(Nrijen,2);

figure
plot(standaardvorm.tvoor,v,'b',standaardvorm.tachter,v,'b');
hold on
grid on
x = [-B/2, -bpiek_fig/(2*24), bpiek_fig/(2*24), B/2]';
y = [0, 1, 1, 0]';
plot(x,y,'r');
ltxt  = [];
...
k_kolom = [-0.40:0.01:1.60]';   %Deze stapgroottes bij voorkeur niet veranderen
...
%Momentane ov.kans Hydra-M
%NOTA BENE: IK NORMEER  DOOR TE DELEN DOOR HET AANTAL OV.DAGEN UIT
%HYDRA-M TE DELEN DOOR HET AANTAL VOOR m = -0.4 m+NAP (dat is helaas niet 182!!!)
mom_ovkans_HM   = OD_HM/OD_HM(1);
stap_HM         = m_HM(2)-m_HM(1);
m_HM_klasmidden = m_HM + 0.5*stap_HM;
klassekansen    = [-diff(mom_ovkans_HM);0];  %geef laatste klassemidden kans 0
mpgem_HM        = sum( klassekansen.*m_HM_klasmidden );
stdev_HM        = (sum( klassekansen.*(m_HM_klasmidden.^2) ) - mpgem_HM^2)^0.5;

disp(['gemiddelde data           = ', num2str(mpgem_data), ' m+NAP'])
disp(['standaardeviatie data     = ', num2str(stdev_data), ' m+NAP'])
disp(['gemiddelde trapezia       = ', num2str(mpgem_trap), ' m+NAP'])
disp(['standaardeviatie trapezia = ', num2str(stdev_trap), ' m+NAP'])
disp(['gemiddelde Hydra-M        = ', num2str(mpgem_HM)  , ' m+NAP'])
disp(['standaardeviatie Hydra-M  = ', num2str(stdev_HM)  , ' m+NAP'])
...
grid on
hold on
plot(m_HM, log(mom_ovkans_HM),'k-')
cltxt  = {'op basis geturfde data','Hydra-VIJ','Hydra-M'};
ltxt  = char(cltxt);
ttxt  = 'Momentane kansen Markermeer (reproductie Hydra-M)';
xtxt  = 'meerpeil, m+NAP';
ytxt  = 'ln momentane overschrijdingskans, [-]';
Xtick = -0.4:0.1:1.0;
Ytick = -18:2:0;
...
%N.B. In het memo van begin okt 2008 wordt voor bepalen van de topduur OD_HM
%gebruikt ipv 182*mom_ovkans (zie eerdere versie van dit hoofdprogramma)
plot(m_HM, 182*mom_ovkans_HM./OF_HM,'k')    
cltxt  = {'Hydra-VIJ','Hydra-M'};
ltxt  = char(cltxt);
ttxt  = 'Overschrijdingsduur per top Hydra-M en Hydra-VIJ Markermeer (reproductie Hydra-M)';
xtxt  = 'meerpeil, m+NAP';
ytxt  = 'topduur, dagen';
Xtick = -0.4:0.1:1.0;
Ytick = 0:5:50;
...
ltxt  = [];
%ttxt  = 'Topduur trapezia Markermeer';
ttxt  = 'Topduur trapezia Markermeer (reproductie Hydra-M)';
xtxt  = 'meerpeil, m+NAP';
ytxt  = 'topduur, uur';
Xtick = -0.4:0.2:1.0;
Ytick = 0:200:1600;
