...
stijging_per_jaar = 0.0011; %aangenomen stijging per jaar in m (regressie van gem geeft 0.0011)

drempel = -0.25;        % variabele voor drempel waarde
...
av        = 0.9999;          %niveau insnoering in verticale richting NB (0.001<= av <=1): av=0 geeft rare antwoorden (av = 0.001 niet)
ah        = 0.9999;          %factor insnoering in horizontale richting (ah = 1 is geen insnoering)
...
plot(wlijn(:,2),wlijn(:,1),'k')  %,'Linewidth', 2);
% ltxt = [];
...
% Nieuwe opmaak
title('Overschrijdingsfrequentie winterhalfjaar Hydra-Zoet (gehomogeniseerde data)')
