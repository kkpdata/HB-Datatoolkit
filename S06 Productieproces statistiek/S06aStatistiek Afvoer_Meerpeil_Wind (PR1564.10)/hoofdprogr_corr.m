...
xmn = 0; xst = 0.01; xmx = 20;
x = (xmn:xst:xmx)';

ymn = -15; yst = 0.01; ymx = 25;
y = (ymn:yst:ymx)';
...
% %invoeren verloop van s in getransformeerde ruimte, (x, s(x)). 
% sinv = [0, 0.2    %linksboven moet beginnen met x = 0; altijd minimaal 1 rij opgeven.
% % 2, 0.3          %getal rechtsonder wordt ook voor hogere x aangenomen als constante
% 30, 9.8]
...
%==========================================================================
% Evt. grote figuur originele ruimte. T.b.v. PR2654.10
%==========================================================================
if geval ==1
    ...
    title(['IJsselmeer tegen IJssel; hulplijnen: Olst 2635 m^3/s, mp = 0.47 m+NAP'])
    ttxt  = ([naamD,' tegen ',naamI]);

    xlim([500, 3000])
    ylim([-0.4, 1])
    xlabel([naamI,' Olst, m^3/s'])
    ylabel([naamD,', m+NAP'])
    
    plot([2635, 2635], [-0.4, 1], 'k')
    plot([0, 3000], [0.47, 0.47], 'k')
...
if geval ==1
    ...
    title(['Correlatie piekafvoer IJssel en IJsselmeer  volgens Hydra-Zoet'])
    ttxt  = ([naamD,' tegen ',naamI]);
