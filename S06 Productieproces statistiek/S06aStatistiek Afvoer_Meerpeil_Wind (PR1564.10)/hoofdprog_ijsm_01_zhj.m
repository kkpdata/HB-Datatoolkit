...
drempel = -0.2;        % variabele voor drempel waarde (was -0.08 m)

zpot = 10;            % zichtduur voor selectie pieken
zB = 10;              %halve breedte van geselecteerde tijdreeks (default=zpot) NB zB>zpot kan soms crash geven

ref_niv = -0.2;       %referentieniveau van waaraf wordt opgeschaald
...
eij = 2005; eim = 12; eid = 31;

%parameters trapezia
B = 20;             %basisduur trapezia
bpiek = 2;          %topduur in dagen

topduur_inv = ...       %DEFAULTKEUZE
    [-0.2, 480;        %eerste kolom moet toenemend zijn, voor tweede geldt dat niet
    -0.05, bpiek*24;
    1.80, bpiek*24]
...
selectie = find(( maand >= 4 & maand <= 9) & (datum >= bedatum & datum <= eidatum));
...
% plot_IJsmgolven_zhj(golven, ref_niv, B, topduur_inv);
...
k_kolom = [-0.4 :0.01:1.0]';   %Deze stapgroottes bij voorkeur niet veranderen
tabelmomkans = [k_kolom, interp1(berek_trap.y, berek_trap.Gy_mom, k_kolom)];
save([padnaam_uit,'momkans_IJsselmeer_berekend_met_stapy_zhj',num2str(stapy),'_ymax',num2str(ymax),'.txt'],'tabelmomkans','-ascii')
...
%==========================================================================
yturf = [(-0.4: stapy : ovkanspiek_inv(1,1))'; berek_trap.y];
