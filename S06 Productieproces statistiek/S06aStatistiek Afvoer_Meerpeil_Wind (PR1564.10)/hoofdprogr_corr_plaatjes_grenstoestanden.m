...
% Dit is een (scratch) afgeleide versie met als doel grenstoestanden in de
% plaatjes te maken.
%==========================================================================
clear
close all
...
30, 9]          %getal rechtsonder wordt ook voor hogere x aangenomen als constante
...
Z1h = [2000, 2000,3000];
Z1v = [1, 0.5, 0.5];
for y = min(Z1v):0.05:max(Z1v)
    plot([min(Z1h),max(Z1h)],[y,y],'k')
end
plot(Z1h, Z1v,'k','Linewidth',3)
ltxt  = [];
...
Z2h = [0, 2000,2000];
Z2v = [0.5, 0.5, -0.4];
plot(Z2h, Z2v,'k','Linewidth',3)
for y = -0.4:0.05:Z1v(2)
    plot([Z2h(2), 3000],[y,y],'k')
end
for y = Z1v(2):0.05:1
    plot([0, 3000],[y,y],'k')
end
