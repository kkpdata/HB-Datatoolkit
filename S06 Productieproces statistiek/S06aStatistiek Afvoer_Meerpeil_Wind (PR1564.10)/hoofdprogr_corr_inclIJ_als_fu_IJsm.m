...
xmn = 0; xst = 0.1; xmx = 20;
x = (xmn:xst:xmx)';

ymn = -15; yst = 0.1; ymx = 25;
y = (ymn:yst:ymx)';
...
% sinv = [0, 0.4    %linksboven moet beginnen met x = 0; altijd minimaal 1 rij opgeven.
% 30, 8.8]          %getal rechtsonder wordt ook voor hogere x aangenomen als constante
...
% %invoeren verloop van s in getransformeerde ruimte, (x, s(x)). 
sinv = [0, 0.4    %linksboven moet beginnen met x = 0; altijd minimaal 1 rij opgeven.
30, 0.4]          %getal rechtsonder wordt ook voor hogere x aangenomen als constante
...
elseif geval == 2 %I = Vecht, D = IJsm
    [parenI, parenD] = textread('paren_V_IJsm_d0_definitief.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [I1, FI2] = textread('F_Vecht.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [D1, FD2] = textread('F_IJsselmeer.txt','%f %f','delimiter',' ','commentstyle','matlab');
    naamI = 'Vecht';
    naamD = 'IJsselmeer';
    imn = min(I1); ist = 5; imx = 900;
    dmn = min(D1); dst = 0.01; dmx = 1.8;
    XtickI = 0:100:600;
    YtickD = -0.4:.2:1;
elseif geval == 3 %I = IJssel, D = Vecht
    [parenI, parenD] = textread('paren_IJ_V_d0_definitief.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [I1, FI2] = textread('F_IJssel.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [D1, FD2] = textread('F_Vecht.txt','%f %f','delimiter',' ','commentstyle','matlab');
    naamI = 'IJssel';
    naamD = 'Vecht';
    imn = min(I1); ist = 25; imx = 3000;
    dmn = min(D1); dst = 5; dmx = 900;
    XtickI = 0:500:3000;
    YtickD = 0:100:600;
elseif geval == 4 %I = Vecht, D = IJssel
    [parenI, parenD] = textread('paren_V_IJ_d0_definitief.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [I1, FI2] = textread('F_Vecht.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [D1, FD2] = textread('F_IJssel.txt','%f %f','delimiter',' ','commentstyle','matlab');
    naamI = 'Vecht';
    naamD = 'IJssel';
    imn = min(I1); ist = 1; imx = 900;
    dmn = min(D1); dst = 5; dmx = 3000;
    XtickI = 0:100:600;
    YtickD = 0:500:3000;
elseif geval == 5 %I = IJsm, D = IJssel
    [parenI, parenD] = textread('paren_IJsm_IJ_d-0.4_definitief.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [I1, FI2] = textread('F_IJsselmeer.txt','%f %f','delimiter',' ','commentstyle','matlab');
    [D1, FD2] = textread('F_IJssel.txt','%f %f','delimiter',' ','commentstyle','matlab');
    naamI = 'IJsselmeer';
    naamD = 'IJssel';
    imn = min(I1); ist = 0.01; imx = 1.8;
    dmn = min(D1); dst = 5; dmx = 3000;
    XtickI = -0.4:.2:1;
    YtickD = 0:500:3000;
...
figure
plot(y, FYs_y); xlabel('y'); ylabel('FY_s(y)');
...
% %==========================================================================
% %Figuur originele ruimte tbv Overzichtsdoc HV en HB
% %==========================================================================
% 
% close all
% 
...
% %==========================================================================
% %Figuur getransformeerde ruimte tbv Belastingmodel TOI
% %==========================================================================
% close all
% 
% figure
...
% plot(x, x+delta+z3*s,'r');
% ltxt  = [];
% ttxt = ['Correlatie tussen IJsselmeer en IJssel: model CS versus metingen']
% xtxt = ['getransformeerde IJsselafvoer [-]'];
% ytxt = ['getransformeerd IJsselmeerpeil [-]'];
% Xtick = 0:1:9;
% Ytick = -2:1:8;
% fig_opmaak_b(ttxt,xtxt,ytxt,ltxt,lpos,Xtick,Ytick,fontsize,linewidth)
