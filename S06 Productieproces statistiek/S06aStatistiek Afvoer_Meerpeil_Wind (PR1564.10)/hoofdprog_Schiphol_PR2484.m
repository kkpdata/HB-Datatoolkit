...
% % % 
% % % Quick and dirty om toe te passen op waterstanden.
% % % 
...
drempel = 154.5;
zpot = 48;            % zichtduur voor selectie pieken
...
Stst = 24;	  % stapgrootte tijd
Sumn = -100;	  % min van windsnelheid
Sust = 100;	  % stapgrootte in windsnelheid
Sumx = 400;	  % max van windsnelheid
...
[jaar, maand, dag,uur,r, snelheid, ws] = textread('waterstanden_uurs_alle_inc_wind2.csv','%f %f %f %f %f %f %f','delimiter',';','headerlines',1);
...
[stormkenmerken, stormen] = stormselectie(drempel,zpot,zB,jaar,maand,dag,uur, ws  ,r,r1,r2,trshift);
%Structure stormen:
...
ws = stormkenmerken(:,6)/10;

N      = numel(ws);
Njaren = 2012 - 1969;

ws_excess = ws - drempel;
par_wbl   = wblfit(ws_excess);

a = par_wbl(1);
b = par_wbl(2);

Ovfreq = N/Njaren * (1-wblcdf(ws_excess, a, b) );
% Ovfreq =(1-wblcdf(ws_excess, a, b) );
figure
semilogy(ws , Ovfreq, 'b.');
grid on
hold on
