...
switch type_data
    ...
    case 'wind'
        ...
        fprintf(fid,'%s\n','*');
        fprintf(fid,'%s\n','* Door: K.A. Wojciechowska van HKV lijn in water');
        fprintf(fid,'%s\n','* Project: PR3280.10');
        fprintf(fid,'%s\n','*');
    ...
    case 'zeewaterstand_Tabel'
        ...
        fprintf(fid,'%s\n','*');
        fprintf(fid,'%s\n','*');
        fprintf(fid,'%s\n','%m+NAP  ZW              WZW             W               WNW             NW              NNW             N');

        fprintf(fid,['%6.1f',repmat('      %1.2e',1,7),' \n'],X');

        fclose all;

    case 'zeewaterstand_Weibull'

        fid = fopen(['OvkansZee_',sNaam,'_',text_data,'.txt'],'wt');
        fprintf(fid,'%s\n','*');
        
        if onzType==0
            fprintf(fid,'%s\n',['* Conditionele overschrijdingskansen zeewaterstand ',sNaam,'. Zonder statistische onzekerheid.']);
        elseif onzType==1
            fprintf(fid,'%s\n',['* Conditionele overschrijdingskansen zeewaterstand ',sNaam,'. Met statistische onzekerheid.']);
        end
        
        fprintf(fid,'%s\n','* WTI2017: 12-uursperiods, gegeven windrichting');
        fprintf(fid,'%s\n','*');
        fprintf(fid,'%s\n','* Door: K.A. Wojciechowska van HKV lijn in water');
        fprintf(fid,'%s\n','* Project: PR3280.10');
        fprintf(fid,'%s\n','*');
