%==========================================================================
% Hoofdprogramma Grevelingen (versie met geknikte trapezia)
% Door: Chris Geerse
% Tbv: PR1322
...
%Data bestanden
infile_ovdag_GR        = 'Grevelingen_mpdag.dat';
infile_ovfreq_GR       = 'Grevelingen_mptop.dat';
infile_data_GR         = 'mp_dagmaxima_GR_1987okt20_2007.txt';
...
bej = 1987; bem = 10; bed = 20;
eij = 2007; eim = 12; eid = 31;    

%Overige parameters
drempel        =  -0.08;  % variabele voor drempel waarde (kies -0.08 voor subplotjes golven)
ref_niv        =  -0.23;  % referentieniveau van waaraf wordt opgeschaald
...
%parameters trapezia
B         = 10;             %basisduur trapezia, neem 10 dagen. N.B. In PR1322 wordt een zichtduur van 10 dagen (B=20) genoemd, maar dat klopt niet.
...
%parameters meerpeil
stapy = 0.001;      %default = 0.01 m. Stapgrootte voor kansvectoren (y is bijv piekafvoer of afvoerniveau)
ymax  = 0.6;        %maximum van vector
...
filenaam_ovdag_GR                           = fullfile(invoerpad, infile_ovdag_GR);
[m_PROM, OD_PROM, OD_PROMwest, OD_PROMoost] = ...
    textread(filenaam_ovdag_GR, '%f %f %f %f','delimiter',' ','commentstyle','matlab');

%m, m+NAP; OF, 1/whjaar. NB: rij meerpeilen moet wel precies hetzelfde zijn
%voor ovfreq en ovdag!
filenaam_ovfreq_GR                 = fullfile(invoerpad, infile_ovfreq_GR);
[m_PROM, OF_PROM]                  = ...
    textread(filenaam_ovfreq_GR,'%f %f','delimiter',' ','commentstyle','matlab');

%--------------------------------------------------------------------------
%Inlezen data
filenaam_data_GR         = fullfile(invoerpad, infile_data_GR);
[jaar, maand, dag, data] = textread(filenaam_data_GR,'%f %f %f %f','delimiter',' ','commentstyle','matlab');
...
%==========================================================================
yturf = [(-0.3: stapy : ovkanspiek_inv(1,1))'; berek_trap.y];
...
hoogste_mp_tabel = 0.4; %NB: moet ruim onder ymax liggen.
Feind            = find(x1 == hoogste_mp_tabel);

tabel = [x1(1:Feind)'; x2(1:Feind)'; x2(1:Feind)'];

fid = fopen([uitvoerpad,'Grevelingenmeer_momentane_ovkansen_v01.txt'],'wt');
