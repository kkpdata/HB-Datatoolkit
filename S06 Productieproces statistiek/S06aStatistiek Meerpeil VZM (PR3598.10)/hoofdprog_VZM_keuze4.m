...
infile_data_VZM         = 'mp_dagmaxima_VZM_1998_2011.txt'; %analyseperiode kan anders zijn!!
...
%Overige parameters
drempel        =  -0.8;   % variabele voor drempel waarde (kies 0.2 voor pieken) -0.099
%%ref_niv        = - 0.10;   %OUDE KEUZE: referentieniveau van waaraf wordt opgeschaald
ref_niv        = 0.05;   %referentieniveau van waaraf wordt opgeschaald
...
%parameters trapezia
B         = 30             %basisduur trapezia MOET hier 30 dagen zijn
...
av        = 0.1;          %niveau insnoering in verticale richting NB (0.001<= av <=1): av=0 geeft rare antwoorden (av = 0.001 niet)
ah        = 0.5;          %factor insnoering in horizontale richting (ah = 1 is geen insnoering)
...
ovkanspiek_inv = ...       
    [ref_niv, 1;
      ...
      0.22   1/6
      0.94,  1e-4/6
      1.12,  1e-5/6];    %laatste toegevoegd voor beter plaatje
% Topduur invoeren (uur)
topduur_inv = ...       
...
%                   Eigenlijk geeft y = 0.001 toch iets andere resultaten.
%                   Maar in Hydra's wordt minder nauwkeurig gerekend dan
%                   stapy = 0.001 m.

ymax  = 2.2;        %maximum van vector
...
%% Hydra-Zoet gegevens inlezen (feitelijk Hydra-NL versie 2.3.0) om te vergelijken met oude gegevens:
...
%% De meest relevante plaatjes voor de rapportage
