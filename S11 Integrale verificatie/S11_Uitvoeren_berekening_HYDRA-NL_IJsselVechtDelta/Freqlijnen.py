...
WMBASE = os.path.join('..', 'Interne_controles')
...
#TRAJECTEN =  [x.split('_')[2] for x in os.listdir(WMBASE)]

for traject in TRAJECTEN:
    if not traject in ['9-1']:
        continue
    print(f'Traject {traject}')
    SAVELOC = os.path.join('..', 'Figuren', 'Frequentielijnen', traject)
    ...
    output = output.loc[output.HydraulicLocationCalculationCollectionEntityId > 4]
    ...
    for loc in tqdm.tqdm(os.listdir(os.path.join(WMBASE, db))):
        ...
        ttijd = 1/output.at[loc, 'TargetProbability'].values
        waarde = output.at[loc, 'Result'].values
        ...
        if type(ttijd) == np.ndarray:
            ...
            langsdata.append([loc, ttijd, waarde_hydra, waarde])
        ax.scatter(ttijd, waarde, color='red', label='Waterstand Riskeer')
        if type(diff) == np.ndarray:
            ...
            xmax =   10**np.ceil(np.log10(ttijd))          
        # limits
        xmin = 10
    ...
    langsdata.to_excel(os.path.join(SAVELOC, f'Dataoverzicht_{traject}.xlsx'))
...
#%%
    ...
    if type(ttijd) == np.ndarray:
        for i, t in enumerate(ttijd):
            ...
            ax.plot(langsdata.vn, hyd, label=f'Hydra-NL T{tt}',
                    linewidth=2, linestyle='-.', color=f'C{i}')
            ax.plot(langsdata.vn, ris, label=f'Riskeer T{tt}' ,
                    linewidth=2, color=f'C{i}')

        ax.set_title(f'Waterstanden {traject} bij verschillende terugkeertijden')
    else:
        ax.plot(langsdata.vn, langsdata['Hydra-NL'], label='Hydra-NL',
                linewidth=2)
        ax.plot(langsdata.vn, langsdata['Riskeer'], label='Riskeer',
                linewidth=2)
        ax.set_title(f'Waterstanden {traject} bij terugkeertijd van {int(ttijd)} jaar')
    ax.set_ylabel('Waterstanden [m+NAP]')
