...
for i=1:size(C{1},1)
    ...
    if ~strcmp(uc, 'U00')
        ...
        if isempty(j)
            ...
            fac = 1.;
            if abs(wlev-9999) <.01 
                fprintf(fid_csv,'%s,%s,%s,%8.4f,%d,%8.4f,%8.4f,%8.2f\n',filecod, C{2}{i}, C{3}{i},C{4}(i),C{5}(i),9999.,9999.,9999.);
                continue;
            end
        elseif j < 2 
            fprintf(fid_log,'%s%s%s%s\n', 'Error - waterlevel not in range wave data; computation: ',strcat(filecod,C{2}{i}),' waterlevel ', num2str(wlev));    
...
%%            continue;
        ...
        if j > 1 
            ...
            fn1_wve = strcat('RMM-',uc,dc,wcod(j,:));
        end
        ...
        if ~strcmp(fn1_wve, fn_old)
            ...
            elseif iret2 ~= 1 
                ...
                fn1_tab = strcat(workdir,'01\RES\',fn1_wve(5:end), '_10.tab');
                fn2_tab = strcat(workdir,'02\RES\',fn2_wve(5:end), '_10.tab');
                [Hs_1, Tm1_1, Dir_1, oeverStat] = readWaveData( fn1_tab, n_oever, oeverStat);
                [Hs_2, Tm1_2, Dir_2, oeverStat] = readWaveData( fn2_tab, n_oever, oeverStat);
            end
        end
        loc_ind = strmatch(strtrim(C{3}{i}),names,'exact');
        Hs = Hs_1(loc_ind) + fac * (Hs_2(loc_ind)-Hs_1(loc_ind));
        Tm1 = Tm1_1(loc_ind) + fac * (Tm1_2(loc_ind)-Tm1_1(loc_ind));
        Dir = Dir_1(loc_ind) + fac * (Dir_2(loc_ind)-Dir_1(loc_ind));
        fprintf(fid_csv,'%s,%s,%s,%8.4f,%d,%8.4f,%8.4f,%8.2f\n',filecod, C{2}{i}, C{3}{i},C{4}(i),C{5}(i),Hs,Tm1,Dir);
    else
        fprintf(fid_csv,'%s,%s,%s,%8.4f,%d,%8.4f,%8.4f,%8.2f\n',filecod, C{2}{i}, C{3}{i},C{4}(i),C{5}(i),0.0,0.0,0.0);
    end
    if mod(i,2000)==0
       disp([num2str(i) ' of '  num2str(size(C{1},1))]);
    end
end
